...
const MAX_UNIX_SOCKET_PATH_BYTES = 99;
...
export function localAgentDaemonPaths(
  ...
  const socketPath = platform !== "win32" && Buffer.byteLength(stateSocketPath) > MAX_UNIX_SOCKET_PATH_BYTES
    ? join(tmpdir(), `devspace-agentd-${hashStateDir(resolvedStateDir)}.sock`)
    : stateSocketPath;
  return {
