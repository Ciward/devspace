...
assert.match(compose, /memswap_limit:\s*24g/);
assert.match(compose, /pids_limit:\s*8192/);
...
assert.match(maintenanceScript, /--reserved-space/);
assert.doesNotMatch(maintenanceScript, /--keep-storage/);
assert.match(maintenanceScript, /DOCKER_CONFIG=.*STATE_ROOT.*docker/);
