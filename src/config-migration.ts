...
const legacyConfigSchema = z.object({
  ...
  worktreeMaxCount: z.number().optional(),
  worktreeArchiveRemote: z.string().optional(),
  ...
  tools: z.object({
    mode: z.enum(["claude", "codex", "full"]).optional(),
    resumableBash: z.boolean().optional(),
    resumableBashYieldMs: z.number().optional(),
  }).strict().optional(),
  ui: z.object({
...
export function migrateLegacyConfig(value: unknown): DevspaceConfig {
  ...
  return devspaceConfigSchema.parse({
    ...
    workspaces: definedEntries({
      ...
      worktreeMaxCount: legacy.worktreeMaxCount,
      worktreeArchiveRemote: legacy.worktreeArchiveRemote,
    }),
