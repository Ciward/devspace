...
try {
  ...
  const longStateDir = resolve(root, "x".repeat(120));
  const longPaths = localAgentDaemonPaths(longStateDir);
  assert.equal(longPaths.stateDir, longStateDir);
  assert.ok(
    Buffer.byteLength(longPaths.socketPath) < 100,
    `Unix daemon socket path is too long: ${longPaths.socketPath}`,
  );
} finally {
