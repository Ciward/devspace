...
```jsonc
{
  ...
  "server": {
    ...
    "allowedHosts": [],
    "trustProxy": false,
    "mcpSessionIdleTimeoutMs": 86400000,
    "mcpSessionCleanupIntervalMs": 300000,
    "mcpSessionMaxCount": 1024,
  },
  "workspaces": {
...
| `full` | Union of the Codex and Claude surfaces plus `grep`, `glob`, and `ls` for host compatibility. |

`tools.resumableBash` makes a long-running `bash` call yield a process session
...
`subagents.maxConcurrentTurns` is a process-wide FIFO concurrency limit. Extra
turns remain queued until a running turn releases a slot. When a provider sets
...
Upstream v1.1 keeps only two user-facing DevSpace environment variables:
...
The `devspace-cheap` branch retains these environment overrides for container
deployments, including the settings listed above plus
`DEVSPACE_MCP_SESSION_IDLE_TIMEOUT_MS`,
`DEVSPACE_MCP_SESSION_CLEANUP_INTERVAL_MS`, `DEVSPACE_MCP_SESSION_MAX_COUNT`,
and `DEVSPACE_SUBAGENT_MAX_CONCURRENT_TURNS`. JSONC remains the durable default;
an explicitly supplied environment variable wins for that process.
